...
import _ss_com.streamsets.pipeline.lib.io.fileref.LocalFileRef;
...
public class TestMarcParserProcessor {
  ...
  public void testProcessor() throws StageException, IOException {
    Path path = Paths.get("src/test/resources/summerland.mrc");


    ProcessorRunner runner = new ProcessorRunner.Builder(MarcParserDProcessor.class)
        .addConfiguration("marcFormat", MarcFormat.ISO_2709)
    ...
    try {
      ...
      FileRef file = new LocalFileRef.Builder().filePath(path.toString()).build();
      fields.put("fileRef", Field.create(file));
      ...
      checkSummerlandContent(record1);
  ...
  @Test(expected = com.streamsets.pipeline.api.StageException.class)
  public void testConfig() throws StageException, IOException {
    Path path = Paths.get("src/test/resources/summerland.mrc");


    ProcessorRunner runner = new ProcessorRunner.Builder(MarcParserDProcessor.class)
        .addConfiguration("marcFormat", null)
        .addOutputLane("output")
        .build();
    runner.runInit();
  }
  ...
  @Test
  public void testXmlProcessor() throws StageException, IOException {
    Path path = Paths.get("src/test/resources/summerland.xml");


    ProcessorRunner runner = new ProcessorRunner.Builder(MarcParserDProcessor.class)
        .addConfiguration("marcFormat", MarcFormat.XML)
        .addOutputLane("output")
        .build();
    runner.runInit();
  ...
  public void testProcessMultiRecords() throws StageException, IOException {
    Path path = Paths.get("src/test/resources/chabon.mrc");
  ...
  @Test
  public void testProcessMultiXmlRecords() throws StageException, IOException {
    Path path = Paths.get("src/test/resources/chabon.xml");
  ...
  private void checkChabonContent(StageRunner.Output output) {
  ...
  private void checkSummerlandContent(Record record) {
    assertEquals(Field.Type.LIST_MAP, record.get().getType());
    Field leader = record.get("/leader");
    assertEquals("00714cam a2200205 a 4500", leader.getValueAsString());
    Field _001 = record.get("/001");
    assertEquals("12883376", _001.getValueAsString());
    Field _005 = record.get("/005");
    assertEquals("20030616111422.0", _005.getValueAsString());
    Field _008 = record.get("/008");
    assertEquals("020805s2002    nyu    j      000 1 eng  ", _008.getValueAsString());

    // Fields 020
    // 020   $a0786808772
    // 020   $a0786816155 (pbk.)
    Field _020 = record.get("/020");
    assertEquals(Field.Type.LIST, _020.getType());
    List<Field> _020List = _020.getValueAsList();
    assertEquals(2, _020List.size());
    assertSame(record.get("/020[0]"), _020List.get(0));
    Field _020_0 = _020List.get(0);
    assertEquals(Field.Type.LIST_MAP, _020_0.getType());
    assertEquals(' ', record.get("/020[0]/indicator1").getValueAsChar());
    assertEquals(' ', record.get("/020[0]/indicator2").getValueAsChar());
    assertEquals(1, record.get("/020[0]/a").getValueAsList().size());
    assertEquals("0786808772", record.get("/020[0]/a[0]").getValueAsString());
    assertEquals(' ', record.get("/020[1]/indicator1").getValueAsChar());
    assertEquals(' ', record.get("/020[1]/indicator2").getValueAsChar());
    assertEquals("0786816155 (pbk.)", record.get("/020[1]/a[0]").getValueAsString());

    //Field 040
    // 040   $aDLC$cDLC$dDLC
    assertEquals(1, record.get("/040").getValueAsList().size());
    assertEquals(' ', record.get("/040[0]/indicator1").getValueAsChar());
    assertEquals(' ', record.get("/040[0]/indicator2").getValueAsChar());
    assertEquals(1, record.get("/040[0]/a").getValueAsList().size());
    assertEquals("DLC", record.get("/040[0]/a[0]").getValueAsString());
    assertEquals(1, record.get("/040[0]/c").getValueAsList().size());
    assertEquals("DLC", record.get("/040[0]/c[0]").getValueAsString());
    assertEquals(1, record.get("/040[0]/d").getValueAsList().size());
    assertEquals("DLC", record.get("/040[0]/d[0]").getValueAsString());

    //Field 100
    //100 1 $aChabon, Michael.
    assertEquals(1, record.get("/100").getValueAsList().size());
    assertEquals('1', record.get("/100[0]/indicator1").getValueAsChar());
    assertEquals(' ', record.get("/100[0]/indicator2").getValueAsChar());
    assertEquals(1, record.get("/100[0]/a").getValueAsList().size());
    assertEquals("Chabon, Michael.", record.get("/100[0]/a[0]").getValueAsString());

    //Field 245
    // 245 10$aSummerland /$cMichael Chabon.
    assertEquals(1, record.get("/245").getValueAsList().size());
    assertEquals('1', record.get("/245[0]/indicator1").getValueAsChar());
    assertEquals('0', record.get("/245[0]/indicator2").getValueAsChar());
    assertEquals(1, record.get("/245[0]/a").getValueAsList().size());
    assertEquals("Summerland /", record.get("/245[0]/a[0]").getValueAsString());
    assertEquals(1, record.get("/245[0]/c").getValueAsList().size());
    assertEquals("Michael Chabon.", record.get("/245[0]/c[0]").getValueAsString());

    //Fields 650
    // 650  1$aFantasy.
    // 650  1$aBaseball$vFiction.
    // 650  1$aMagic$vFiction.
    assertEquals(3, record.get("/650").getValueAsList().size());
    assertEquals(' ', record.get("/650[0]/indicator1").getValueAsChar());
    assertEquals('1', record.get("/650[0]/indicator2").getValueAsChar());
    assertEquals(1, record.get("/650[0]/a").getValueAsList().size());
    assertEquals("Fantasy.", record.get("/650[0]/a[0]").getValueAsString());
    assertEquals(' ', record.get("/650[1]/indicator1").getValueAsChar());
    assertEquals('1', record.get("/650[1]/indicator2").getValueAsChar());
    assertEquals(1, record.get("/650[1]/a").getValueAsList().size());
    assertEquals("Baseball", record.get("/650[1]/a[0]").getValueAsString());
    assertEquals(1, record.get("/650[1]/v").getValueAsList().size());
    assertEquals("Fiction.", record.get("/650[1]/v[0]").getValueAsString());
    assertEquals(' ', record.get("/650[2]/indicator1").getValueAsChar());
    assertEquals('1', record.get("/650[2]/indicator2").getValueAsChar());
    assertEquals(1, record.get("/650[2]/a").getValueAsList().size());
    assertEquals("Magic", record.get("/650[2]/a[0]").getValueAsString());
    assertEquals(1, record.get("/650[2]/v").getValueAsList().size());
    assertEquals("Fiction.", record.get("/650[2]/v[0]").getValueAsString());
  }
